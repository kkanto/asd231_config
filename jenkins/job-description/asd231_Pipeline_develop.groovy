...
def customLib = library identifier: 'asd231_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asd231_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asd231.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asd231", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asd231_Pipeline_develop");

job.with {

    authenticationToken('EDvZt5mz3tl1SpkWLoAx6V5vVngdBsuW')
